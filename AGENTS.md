...
- Keep the original Particle Background (`includes/element-particle.php`, `assets/script.js`, `assets/main.css`) on its bundled particles.js runtime. Do not fold Logo Gather behavior into it.
- Particle Logo Gather is a separate nestable custom Canvas element at `includes/element-particle-logo-gather.php`; its isolated `plg-*` runtime and styles are `assets/particle-logo-gather.js` and `assets/particle-logo-gather.css`. Its handles must be registered and reliably enqueued for both rendered Bricks pages and builder preview; bump both handle versions whenever either asset changes.
- Logo Gather accepts a documented particles.js-style subset, not full particles.js compatibility. Logo alpha-mask sampling requires an uploaded same-origin image or a cross-origin image with suitable CORS headers; always test missing/invalid images, the center gather/release state, gather-only dot count, and mobile/tablet suppression.
- Build distributable archives with `scripts/package-plugin.sh`; it reads the plugin header version and writes `dist/bricks-elements-pack-<version>.zip`. Run it after version changes and verify that its archive excludes `.git`, `.slim`, and local guidance artifacts.
